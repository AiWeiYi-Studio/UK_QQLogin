...
if ($_GET['code']) {
	...
	if($row['access_token']=='') {
		...
		$DB->query("insert into `qqlogin_logs` (`uid`,`type`,`date`,`city`,`data`) values ('".未登录."','使用QQ登录平台,未成功(该QQ未绑定)','".$date."','".$city."','IP:".$clientip."')");
		@header('Content-Type: text/html; charset=UTF-8');
		...
		$DB->query("insert into `qqlogin_logs` (`uid`,`type`,`date`,`city`,`data`) values ('".$user."','使用QQ登录平台','".$date."','".$city."','IP:".$clientip."')");
