...
echo '
<div class="block">
<div class="block-title"><h3 class="panel-title">站长操作日志</h3></div>
<div class="card-body">
...
if($my=='search') {
	...
	$numrows=$DB->count("SELECT count(*) from qqlogin_logs WHERE{$sql}");
	$con='包含 '.$_GET['value'].' 的共有 <b>'.$numrows.'</b> 条记录';
	$link='&my=search&column='.$_GET['column'].'&value='.$_GET['value'];
}else{
	$numrows=$DB->count("SELECT count(*) from qqlogin_logs");
	$sql=" 1";
...
$rs=$DB->query("SELECT * FROM qqlogin_logs WHERE{$sql} order by date desc limit $offset,$pagesize");
