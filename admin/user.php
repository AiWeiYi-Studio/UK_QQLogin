...
$power=$_POST['power'];
$name=$_POST['name'];
$access_token=$_POST['name'];
$ip=$_POST['ip'];
if($user==NULL or $pwd==NULL or  $qq==NULL or $active==NULL or $power==NULL or $name==NULL){
showmsg('保存错误,请确保每项都不为空!',3);
} else {
$rows=$DB->get_row("select * from qqlogin_user where user='$user' limit 1");
if($rows)
exit("<script language='javascript'>alert('站长名已存在！');window.location.href='user.php?my=add';</script>");
$sql="insert into `qqlogin_user` (`user`,`pass`,`qq`,`active`,`power`,`name`,`boss`,`access_token`,`ip`) values ('".$user."','".$pwd."','".$qq."','".$active."','".$power."','".$name."','".$udata['uid']."','".$access_token."','".$ip."')";
if($DB->query($sql)){
...
if($DB->query("update qqlogin_user set user='$user',pass='$pwd',qq='$qq',active='$active',power='$power',name='$name',ip='$ip',access_token='$access_token' where uid='{$id}'"))
...
{
$id=intval($_GET['id']);
$sql="DELETE FROM qqlogin_user WHERE uid='$id'";
	$city=get_ip_city($clientip);
	$DB->query("insert into `ukyun_log` (`uid`,`type`,`date`,`city`,`data`) values ('".$user."','删除站长','".$date."','".$city."','IP:".$clientip."')");
if($DB->query($sql))
...
$numrows=$DB->count("SELECT count(*) from qqlogin_user");
if(isset($_GET['id'])){
	$sql = " uid={$_GET['id']}";
...
?>
      <div class="table-responsive">
        <table class="table table-striped">
          <thead><tr><th>UID</th><th>昵称</th><th>账号</th><th>密码</th><th>QQ</th><th>状况</th><th>权限</th><th>操作</th></tr></thead>
          <tbody>
<?php
$pagesize=30;
...
$rs=$DB->query("SELECT * FROM qqlogin_user WHERE{$sql} order by uid desc limit $offset,$pagesize");
