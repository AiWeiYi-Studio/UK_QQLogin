...
if(isset($_GET['qq'])) {
	$qq=intval($_GET['qq']);
	$sql=" `qq`='{$qq}'";
	$gls=$DB->count("SELECT count(*) from qqlogin_site WHERE{$sql}");
	$con='QQ('.$qq.')共有 <b>'.$gls.'</b> 个域名';
	$link='&id='.$_GET['uid'];
}else{
	$gls=$DB->count("SELECT count(*) from qqlogin_site WHERE 1");
	$sql=" 1";
	$con='平台共有 <b>'.$gls.'</b> 个分发<br/><a href="./addurl.php" class="btn btn-primary">添加分发</a>&nbsp;<a href="#" data-toggle="modal" data-target="#searchqq" id="searchqq" class="btn btn-success">搜索QQ</a>';
}
