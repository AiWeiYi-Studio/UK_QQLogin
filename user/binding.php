...
if ($_GET['code']) {
	...
	$row = $DB->get_row("SELECT * FROM qqlogin_daili WHERE access_token='$access_token' limit 1");
		if($row['access_token']==$access_token) {
		$city=get_ip_city($clientip);
		$DB->query("insert into `qqlogin_log` (`uid`,`type`,`date`,`city`,`data`) values ('".$user."','绑定QQ,但未绑定成功(QQ已被使用)','".$date."','".$city."','IP:".$clientip."')");
		@header('Content-Type: text/html; charset=UTF-8');
		exit("<script language='javascript'>alert('该QQ已绑定其他用户！');history.go(-1);</script>");
	}
	$DB->query("update `qqlogin_daili` set `access_token` ='".$access_token."' where `uid`='".$udata['uid']."'");
	$city=get_ip_city($clientip);
		$DB->query("insert into `qqlogin_log` (`uid`,`type`,`date`,`city`,`data`) values ('".$user."','绑定QQ,已成功','".$date."','".$city."','IP:".$clientip."')");
	@header('Content-Type: text/html; charset=UTF-8');
		exit("<script language='javascript'>alert('恭喜你，绑定成功！');window.location.href='./';</script>");
}elseif(isset($_GET['jiebang'])){
		$DB->query("update `qqlogin_daili` set `access_token` ='' where `uid`='".$udata['uid']."'");
		$city=get_ip_city($clientip);
		$DB->query("insert into `qqlogin_log` (`uid`,`type`,`date`,`city`,`data`) values ('".$user."','解绑QQ,已成功','".$date."','".$city."','IP:".$clientip."')");
		@header('Content-Type: text/html; charset=UTF-8');
