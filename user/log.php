...
if($my=='search') {
	...
	$numrows=$DB->count("SELECT count(*) from qqlogin_log WHERE{$sql}");
...
?>
      <div class="table-responsive">
        <table class="table table-striped">
          <thead><tr><th>操作类型</th><th>时间</th><th>城市</th><th>数据</th></tr></thead>
          <tbody>
<?php
$pagesize=30;
...
$rs=$DB->query("SELECT * FROM qqlogin_log WHERE{$sql} order by date desc limit $offset,$pagesize");
while($res = $DB->fetch($rs))
{
echo '<tr><td>'.$res['type'].'</td><td>'.$res['date'].'</td><td>'.$res['city'].'</td><td>'.$res['data'].'</td></tr>';
}
?>
          </tbody>
        </table>
